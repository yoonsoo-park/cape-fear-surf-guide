...
The judge-facing endpoint is an unauthenticated, read-only HTTPS MCP service.
...
API Gateway also targets 0.2 requests per second with a burst of two. That
throttle smooths bursts but is not a cost guarantee.
...
Lambda has reserved concurrency 2, a 20-second timeout, a bounded request
body, no VPC, and narrowly scoped record-table and exposure-control-table IAM.

The API and application do not log request bodies, party profiles, or
`Authorization` values. API Gateway method execution logging is disabled.
There is no Lambda Function URL, bearer token, SecureString, token-rotation
path, Cognito configuration, Google credential, OAuth 2.1 implementation, or
local Keychain dependency in this public deployment.
...
smoke-test owner, `BudgetAlertEmail`, exact `PublicUntilUtc`, and the public
endpoint's operating responsibility. The approver must also provide a new
`ExposureId` for this public window.
...
At expiry, record the circuit-breaker reason and verify public Lambda reserved
...
Claude Desktop is the primary manual demonstration route. Use the released
remote MCP connector UI to add the approved public `POST /mcp` URL, discover
...
Run the same three checks in ChatGPT Developer Mode only on an account where
remote MCP is available. That is compatibility evidence, not a compatibility
guarantee. If either product's current connector cannot make the request,
record it as not verified rather than adding OAuth or weakening the public
controls.
...
consent, token-storage, rotation, and client-compatibility design. This demo
does not claim that no-auth access is a production authorization model.
